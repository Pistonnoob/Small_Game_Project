...
bool ParticleShaderHandler::Initialize(ID3D11Device* device, HWND* hwnd)
{
	...
	hresult = D3DCompileFromFile(vsFilename, NULL, NULL, "main", "vs_5_0", D3D10_SHADER_OPTIMIZATION_LEVEL3, 0, &vertexShaderBuffer, &errorMessage);
	if (FAILED(hresult)) {
		if (errorMessage) {
	...
	hresult = D3DCompileFromFile(gsFilename, NULL, NULL, "main", "gs_5_0", D3D10_SHADER_OPTIMIZATION_LEVEL3, 0, &geoShaderBuffer, &errorMessage);
	if (FAILED(hresult)) {
		if (errorMessage) {
	...
	hresult = D3DCompileFromFile(psFilename, NULL, NULL, "main", "ps_5_0", D3D10_SHADER_OPTIMIZATION_LEVEL3, 0, &pixelShaderBuffer, &errorMessage);
	if (FAILED(hresult)) {
		if (errorMessage) {
